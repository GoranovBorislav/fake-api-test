...
describe(`Endpoint /${endpoint}`, () => {
    ...
    const deleteAuthorsTests = [
        { parameter: 12, expectedCode: HttpStatusCode.Ok, checkRepository: true },
        { parameter: -12, expectedCode: HttpStatusCode.NotFound, checkRepository: false },
    ];
    ...
    const putAuthorsTests = [
        { titleSlug: '', parameter: 14, expectedCode: HttpStatusCode.Ok, payload: true },
        { titleSlug: '', parameter: -33, expectedCode: HttpStatusCode.NotFound, payload: true },
        { titleSlug: 'with undefined payload', parameter: 5, expectedCode: HttpStatusCode.UnsupportedMediaType, payload: false }
    ];
    putAuthorsTests.forEach((test) => {
        describe(`${Method.PUT} ${endpoint} by Id: ${test.parameter} ${test.titleSlug}`, () => {
            let response: AxiosResponse<any, any> | AxiosResponse<unknown, any>;
            const payload = test.payload ? repository.generateAuthorPayload() : undefined;
    ...
    describe(`${Method.POST} ${endpoint}`, () => {
        let response: AxiosResponse<any, any> | AxiosResponse<unknown, any>;
        const payload = repository.generateAuthorPayload();
        ...
        it('has updated the repository', async () => {
            const configuration = api.buildRequestConfiguration(Method.GET, endpoint, { id: (response.data as Author).id });
            response = await api.sendRequest(configuration);
            expect(response.status).to.equal(HttpStatusCode.Ok);
            expect(response.data).to.contain(payload);
        });
    ...
    describe(`${Method.POST} ${endpoint} with dublicated Id`, () => {
        let response: AxiosResponse<any, any> | AxiosResponse<unknown, any>;

        before(async () => {
            const configuration = api.buildRequestConfiguration(Method.POST, endpoint, { payload: existingAuthor });
            response = await api.sendRequest(configuration);
        });
